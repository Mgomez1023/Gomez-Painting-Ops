...
class SocialQueueService:
            ...
            for slot in slots:
                ...
                queue_items.append(
                    self._build_queue_item(
                        ...
                        caption=self._apply_emoji_preference(
                            self._caption_for_platform(draft_set, slot.platform),
                        ...
                        week_start_date=week_start_date,
                        business_id=request.business_id,
                        emoji_preference=request.emoji_preference,
                    )
                )
    ...
    def generate_manual_post(
        ...
        queue_item = self._build_queue_item(
            ...
            platform=platform,
            caption=self._apply_emoji_preference(
                self._caption_for_platform(draft_set, platform),
                request.emoji_preference,
                campaign,
            ),
            post_type=request.post_type,
    ...
    def _campaign_for_request(
        ...
        if business_profile is not None and not campaign_id:
            return self._campaign_from_business_profile(business_profile, business_id=business_id)
    ...
    @staticmethod
    def _campaign_from_business_profile(profile: BusinessProfileInput, business_id: str | None = None) -> Campaign:
        business_name = profile.business_name.strip() or "Marom Painting"
        normalized_business_id = SocialQueueService._business_profile_campaign_suffix(business_id)
        ...
        return Campaign(
            campaign_id=f"BUSINESS-PROFILE-{normalized_business_id}" if normalized_business_id else "BUSINESS-PROFILE",
    ...
    @staticmethod
    def _business_profile_campaign_suffix(business_id: str | None) -> str:
        return re.sub(r"[^A-Za-z0-9_-]+", "-", (business_id or "").strip()).strip("-")
    ...
    def _build_queue_item(
        ...
        note_lines = [
            ...
            f"Business ID: {business_id or ''}",
            f"Emoji Preference: {emoji_preference}",
    ...
    @classmethod
    def _apply_emoji_preference(cls, caption: str, preference: EmojiPreference, campaign: Campaign) -> str:
        clean_caption = caption.strip()
        if not clean_caption:
            return caption
        emoji_count = {"less": 1, "more": 4}.get(preference, 2)
        emoji_prefix = " ".join(cls._emoji_candidates(campaign)[:emoji_count])
        return f"{emoji_prefix} {clean_caption}"

    @staticmethod
    def _emoji_candidates(campaign: Campaign) -> list[str]:
        search_text = " ".join(
            [
                campaign.campaign_name,
                campaign.service_focus,
                campaign.target_customer,
