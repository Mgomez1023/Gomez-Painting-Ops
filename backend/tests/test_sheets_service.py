...
def test_save_campaign_draft_set_to_queue_appends_five_review_rows() -> None:
    ...
    draft_set = CampaignDraftSet(
        facebook_post="Facebook draft",
        facebook_group_post="Facebook Groups draft",
        google_business_post="Google Business draft",
        instagram_caption="Instagram draft",
    ...
    assert len(service.appended_rows) == 6
...
def test_update_post_scheduled_at_updates_matching_posts_sheet_row() -> None:
    ...
    assert "Scheduled Date: 2026-05-01" in updated_post.notes
    assert "Week: 2026-W18" in updated_post.notes
    assert "Week Start: 2026-04-27" in updated_post.notes
    assert "Day of Week: Friday" in updated_post.notes
    assert "Scheduled Date: 2026-04-29" not in updated_post.notes
    assert service.updated_row_number == 2
    assert service.updated_item == updated_post


def test_update_post_image_updates_matching_posts_sheet_row() -> None:
    class FakeSheetsService(SheetsService):
        updated_row_number: int | None = None
        updated_item: CampaignContentQueueItem | None = None
    ...
    service = FakeSheetsService()
    updated_post = service.update_post_image(
        "WSQ-1001-meta-dual-1",
    ...
    assert updated_post is not None
    assert updated_post.image_filename == "interior-refresh.jpg"
    assert updated_post.image_path == "/media/photo-assets/interior-refresh.jpg"
    assert updated_post.image_url is None
    assert updated_post.notes == "Generated by Weekly Social Queue"
    assert service.updated_row_number == 2
    assert service.updated_item == updated_post


def test_campaign_content_queue_item_to_row_for_headers_preserves_extra_columns() -> None:
    item = CampaignContentQueueItem(
        content_id="WSQ-1001-meta-dual-1",
        campaign_id="CAMP-1001",
        platform="Meta Dual",
        draft_text="Weekly post",
        cta="Request a quote",
        landing_page_url="https://marompainting.org",
        image_filename="kitchen.webp",
        image_path="/media/kitchen.webp",
        image_url="/media/kitchen.webp",
    ...
    row = SheetsService._campaign_content_queue_item_to_row_for_headers(
        item=item,
        headers=headers,
        existing_row=existing_row,
    )

    assert row[0] == "WSQ-1001-meta-dual-1"
    assert row[4] == "=ROW()"
    assert row[5] == "2026-05-01T12:00:00Z"
    assert row[6] == "Marom Painting"
    assert row[7] == "Seasonal Reminder"
